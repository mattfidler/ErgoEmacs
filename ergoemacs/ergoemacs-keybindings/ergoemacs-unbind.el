  ...
  :type '(repeat
          (list :tag "Old Emacs Key"
                (choice (string :tag "Kbd Code")
                        (sexp :tag "Key"))
                (repeat
                 (choice (const :tag "Prefix Argument" 'prefix)
...
(defun ergoemacs-unset-redundant-global-keys ()
  ...
  (mapc (lambda (x)
          (let ((current-key (lookup-key (current-global-map) (read-kbd-macro (encode-coding-string (nth 0 x) locale-coding-system)) t)))
            (when (keymapp current-key)
                (setq current-key 'prefix))
            (when (memq current-key (nth 1 x))
              (unless (member (key-description
            ...
            ;; Do not unset key since already non-default.
            )))
...
(defun ergoemacs-restore-global-keys ()
  ...
  (mapc (lambda (x)
          (unless (and ;;(not (keymapp (edmacro-parse-keys (car (cdr x)))))
                       (member (key-description (edmacro-parse-keys (car (cdr x)))) ergoemacs-globally-defined-keys))
            (define-key
              (car x)
